...
import model.RADDet.model.layers as L
...
def radarVGG3D(x, ):
    ...
    ##### VGG16 3D #####
    ### Block 1
    conv = twoConvLayersBlock(conv, int(conv.shape[-1] * filters_expansions[0]), if_pool=False)
    ### Block 2
    conv = twoConvLayersBlock(conv, int(conv.shape[-1] * filters_expansions[1]), if_pool=True)
    ### Block 3

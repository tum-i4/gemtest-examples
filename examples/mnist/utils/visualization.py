...
def plotHistory(history):
    ...
    """
    # Extracting keys for loss and accuracy
    loss_keys = [key for key in history.history.keys() if 'loss' in key]
    accuracy_keys = [key for key in history.history.keys() if 'accuracy' in key]

    # Plotting loss curves
    pd.DataFrame(history.history)[loss_keys].plot(figsize=(8, 5))
    plt.title("Loss Curves")
    plt.grid(True)
    plt.show()

    # Plotting accuracy curves
    pd.DataFrame(history.history)[accuracy_keys].plot(figsize=(8, 5))

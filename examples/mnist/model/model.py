...
from examples.mnist.model import efficient_capsnet_graph_mnist, efficient_capsnet_graph_smallnorb, \
    efficient_capsnet_graph_multimnist, original_capsnet_graph_mnist
from examples.mnist.utils import pre_process_multimnist
from examples.mnist.utils.dataset import Dataset
from examples.mnist.utils.pre_process_mnist import pre_process
from examples.mnist.utils.tools import get_callbacks, marginLoss, multiAccuracy
...
class Model(object):
    ...
    # amir added
    def evaluate_image(self, input_data):
        """
        Use the model to predict a single input ndarray.

        Parameters:
        - input_data: ndarray
            Single input data for prediction.
        ...
        if np.max(input_data) > 1:
            input_data, _ = pre_process(input_data, 1)
        input_data = np.expand_dims(input_data, axis=0)  # Add batch dimension
        output = self.model.predict(input_data)
        prediction_probs = output[0][0]

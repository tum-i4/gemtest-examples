...
class MnistCNN(nn.Module):
    """
    Defines custom vanilla model for MNIST dataset.
    """
    model_relative_path = "bin/mnist_training/mnist_cnn"
    ...
    def forward(self, x):
        """
        Implement model forward pass. Structure of the model defined in this function
        """
        x = self.conv1(x)
        x = F.relu(x)
        x = self.conv2(x)

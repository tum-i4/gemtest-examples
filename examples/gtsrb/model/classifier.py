...
def read_traffic_signs(rootpath: str = "data/") -> List[ndarray]:
    ...
    data_dir: Path = Path(__file__).parent.parent / rootpath
    prefix: Path = data_dir / "GTSRB_Images" / "GTSRB" / "Final_Test" / "Images"

    with open(
            data_dir / "GTSRB_Metadata" / "GT-final_test.csv", encoding="utf-8"
    ) as gt:
...
class TrafficSignClassifier(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.load_state_dict(
            torch.load(
                Path(__file__).parent / "bin/classifier_weights/classifier_pretrained_weights.pth",
